...
def get_h3_index_at_resolution(h3_index, resolution):
    current_resolution = h3.h3_get_resolution(h3_index)
    
    if resolution > current_resolution:
        # Convert to a finer resolution
        raise ValueError("Cannot directly get a higher resolution parent. Use `h3.h3_to_children()` for this.")
...
def compute_distance_matrix(spatial_tessellation, origins):
    ...
    if not hasattr(spatial_tessellation, 'geometry'):
        raise ValueError("The provided spatial_tessellation must have a 'geometry' column.")

    #centroids = spatial_tessellation.geometry.apply(lambda geom: (geom.centroid.y, geom.centroid.x)).values
    centroids = spatial_tessellation['index'].apply(
        lambda h3_index: h3_index_to_centroid(h3_index)
        ).apply(
    ...
    for id_i in tqdm(range(n)): #origins
        ...
        for id_j in range(id_i + 1, n):
            ...
            distance = geodesic((lat_i, lng_i), (lat_j, lng_j)).kilometers #.kilometers
...
def h3_to_polygon(h3_index):
    boundary = h3.h3_to_geo_boundary(h3_index, True)  # True indicates the GeoJSON coordinate standard (latitude, longitude) used when returning geo coordinates
...
class Gravity:
    def __init__(self, 
        ...
        if self.deterrence_func_type == 'power_law':
            ...
            raise ValueError(f"Invalid deterrence function type: {self.deterrence_func_type}")
            # exponent = self.deterrence_func_args[0]
            # self.deterrence_function = lambda d: d ** exponent
    ...
    def get_gravity_score(
        ...
        trip_probs_matrix = np.nan_to_num(trip_probs_matrix, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def from_matrix_to_flowdf(self, flow_matrix, origins, spatial_tessellation):
        ...
        index2tileid = {index: spatial_tessellation.iloc[index]['index'] for index in range(len(spatial_tessellation))}
    ...
    def generate(
        ...
        trip_probs_matrix = self.get_gravity_score(distance_matrix, relevances, relevances)
        ...
        if self.gravity_type == "global":
            ...
            if out_format == "probabilities":
                flow_df = self.from_matrix_to_flowdf(trip_probs_matrix, origins, spatial_tessellation)
                return flow_df
                ...
                flow_df = self.from_matrix_to_flowdf(od_matrix, origins, spatial_tessellation)
                return flow_df
            ...
            if out_format == "probabilities":
                ...
                flow_df = self.from_matrix_to_flowdf(avg_flows_matrix, origins, spatial_tessellation)
                return flow_df
    ...
    def fit(
        ...
        if self.is_h3_hexagon:
            ...
            h3center_mapping = {h3_index: h3_index_to_centroid(h3_index) for h3_index in unique_h3_index}
        ...
        flow_df.progress_apply(lambda flow_example: self.update_training_set(flow_example), axis=1)
        ...
        model = GLM(self.y, self.X, family=sm.genmod.families.family.Poisson(link=sm.genmod.families.links.Log()))
    ...
    def update_training_set(self, flow_example):
        ...
        try:
            ...
            print('Missing infor for location ' + \
                    '"%s" Skipping ...' % origin_id)
            return
        ...
        if self.gravity_type == 'global':
            ...
            sc_vars = create_normalization_constraint_vector(self.tileid2index[origin_id], len(self.tileid2index))

        if self.deterrence_func_type == 'exponential':
            # exponential deterrence function
            self.X += [[1.] + sc_vars + [np.log(destination_relevance), - distance]]
        elif self.deterrence_func_type == 'power_law':
            # power law deterrence function
            self.X += [[1.] + sc_vars + [np.log(destination_relevance), np.log(distance)]]
...
def get_outflow_hex_df(
    ...
    flow_hex_df = flow_hex_df.rename(columns={
    ...
    resolution_mapping = {h3_index: get_h3_index_at_resolution(h3_index, hex_resolution) for h3_index in unique_h3_indices}
    # Update origin and destination columns with new resolution
    flow_hex_df['origin'] = flow_hex_df['origin'].map(resolution_mapping)

    # Obtain outflow data
    outflow_hex_df = flow_hex_df.groupby('origin', as_index=False).agg({'total_outflow': 'sum'})
    ...
    outflow_hex_df_with_pop = pd.merge(filtered_outflow_df, pop_df, left_on='origin', right_on='index', how='left')
...
def get_actual_flow_hex_df(
    ...
    if pop_df is not None:
        ...
        filtered_flow_df = flow_hex_df[
            flow_hex_df['origin'].isin(valid_indices) & flow_hex_df['destination'].isin(valid_indices)
        ]
...
def obtain_population_data(state_fips_codes, county_fips_codes, hex_resolution, year, 
                           census_dataset, projection_crs, census_variables):
    pop_list = []
    for state_fips in state_fips_codes:
        for county_fips in county_fips_codes.get(state_fips, []):
            try:
                pop_df_i = county_tesslation.tesselate_county(
                ...
                pop_list.append(pop_df_i)
            except Exception as e:
                print(f"Failed to process state {state_fips}, county {county_fips}: {e}")
...
def obtain_relevance_data_at_resolution(
    ...
    relevance_df = relevance_df.groupby('index', as_index=False).agg(stay_count=(colname_caid, 'count'))
