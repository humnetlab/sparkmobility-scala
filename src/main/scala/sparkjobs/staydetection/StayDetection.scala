...
object StayDetection {
  ...
  def mapToH3(
      data_i: DataFrame,
      resolution: Int,
      temporalThreshold: Int = 3600,
      filterPassing: Boolean = true,
      speed_threshold: Double = 30.0
  ): (DataFrame, DataFrame) = {
    ...
    val DataWithH3 = data_i.withColumn(
      "h3_index",
      latLonToH3UDF(col("latitude"), col("longitude"), lit(resolution))
    )
    // val DataWithH3 = data_i.withColumn("h3_index", LatLonToH3(col("latitude"), col("longitude"), lit(resolution)))

    val DataWithPrevH3 = DataWithH3.withColumn(
      "prev_h3_index",
      lag("h3_index", 1).over(windowSpec)
    )
    val DataWithPrevEndTime = DataWithPrevH3.withColumn(
      "prev_h3_stay_end_time",
      lag("stay_start_timestamp", 1).over(windowSpec)
    )

    // Check if the user has entered a new H3 cell or if the stay time exceeds the threshold
    val DataWithStayIndex = DataWithPrevEndTime.withColumn(
      "stay_index_h3",
      when(
        (col("h3_index").isNull) ||
          (col("prev_h3_index") =!= col("h3_index")) ||
          (col("prev_h3_index") === col("h3_index") && (unix_timestamp(
    ...
    val DataWithCumulativeIndex = DataWithStayIndex.withColumn(
      "stay_index_h3",
      sum(col("stay_index_h3")).over(windowSpec)
    )
    ...
    val aggregated = DataWithEndTimestamp
      .groupBy("caid", "stay_index_h3")
      .agg(
        min("stay_start_timestamp").alias("h3_stay_start_time"),
        max("stay_end_timestamp").alias("h3_stay_end_time"),
        min("h3_index").alias("h3_id"),
        avg("latitude")
          .alias("h3_stay_lat"), // Average latitude for stays in this H3 cell
        avg("longitude")
          .alias("h3_stay_lon") // Average longitude for stays in this H3 cell
      )

    // If filterPassing is true, apply logic to filter passing points
    if (filterPassing) {
      val passingWindowSpec =
        Window.partitionBy("caid").orderBy("h3_stay_start_time")
      ...
      val checkWithPreviousNext = aggregated
        ...
        .withColumn(
          "prev_h3_stay_end_time",
          lag("h3_stay_end_time", 1).over(passingWindowSpec)
        )
        .withColumn("next_h3_id", lead("h3_id", 1).over(passingWindowSpec))
        .withColumn(
          "next_h3_stay_start_time",
          lead("h3_stay_start_time", 1).over(passingWindowSpec)
        )

      // Use checkPassing to filter according to the speed limit
      val checkedData = checkWithPreviousNext.withColumn(
        "passing",
        checkPassing(
          col("prev_h3_id"),
          col("next_h3_id"),
      ...
      try {
        ...
        if (!h3LookupDict.contains(h3_id)) {
          ...
          val elements = h3
            .gridDisk(h3Index, 1)
            .asScala
            .map(_.toString) // Convert Long back to String
  ...
  def mergeH3Region(df: DataFrame, params: FilterParametersType): DataFrame = {
    ...
    val result = dfWithLag
      ...
      .agg(
        ...
        (max("h3_stay_end_time") - min("h3_stay_start_time"))
          .alias("stay_duration"),
    ...
    val withUtc = result
      .withColumn(
        "stay_start_ts_utc",
        col("stay_start_timestamp").cast("timestamp")
      )
      .withColumn(
        "stay_end_ts_utc",
        col("stay_end_timestamp").cast("timestamp")
      )
    ...
    val withLocal = withUtc
      .withColumn(
        "local_time",
        from_utc_timestamp(col("stay_start_ts_utc"), params.timeZone)
      )
