...
object dataLoadFilter {
  def castToUTCTimestamp(
    ...
    df.withColumn(
      ...
      when(
        col(columnName)
          .cast("string")
          .rlike("^[0-9]+$"), // Check if it's a Unix timestamp
        to_utc_timestamp(
          from_unixtime(col(columnName).cast("long")),
          timeZone
        ) // Convert Unix to UTC timestamp
      ).otherwise(
        to_utc_timestamp(
          col(columnName).cast("timestamp"),
          timeZone
        ) // Cast to UTC timestamp if not Unix
      )
    )
  }
  ...
  def timeFrame(
    ...
    val startTimeUTC = LocalDateTime
      .parse(params.startTimestamp, formatter)
      .atOffset(ZoneOffset.UTC)
      .format(formatter)
    val endTimeUTC = LocalDateTime
      .parse(params.endTimestamp, formatter)
      .atOffset(ZoneOffset.UTC)
      .format(formatter)
